...
{
    static final SecretSign cl_ipsCrypt = new SecretSign();
    ...
    static transient Random theRandom = new Random();

    private static String algorithm = "DESede";
    private static Key key = null;
    private static Cipher cipher = null;

    public static synchronized String checkPassword(String key, String salt)
    {
        return jCrypt(salt, key);
    }
    ...
    private static final String jCrypt(String salt, String original)
    {
        while (salt.length() < 2) salt = salt + "A";
    ...
    {
        SecretSign s = new SecretSign();
        System.out.println(checkPassword("123","111111111111111"));
        //求解密
    }

    public static String getSecPass(String pass)
    {
        if ((pass == null) || (pass == "")) {
            return null;
        }
        String salt = new BigInteger(32, theRandom).toString(36);
        if (salt.length() < 2)
            salt = "xx";
        if (salt.length() > 2)
            salt = salt.substring(1, 3);
        String sec_pass = checkPassword(pass, salt);
        return sec_pass;
    }

    public static String getEnc(String key)
    {
        String seed = key.length() > 2 ? key.substring(0, 2) : key;
        String retVal = "";

        if (key.length() > 8) {
            String str0 = key.substring(0, 8);
            String str1 = key.substring(8);
            String result = checkPassword(str0, seed);
            retVal = result + getEnc(str1);
        } else {
            retVal = checkPassword(key, seed);
        }
    ...
    private static byte[] encrypt(String input)
            throws InvalidKeyException, BadPaddingException, NoSuchAlgorithmException, NoSuchPaddingException, IllegalBlockSizeException
    {
